...
/**
 * Saisie d'un intérimaire
 */
public static void SaisirInterimaire(){
		
		ArrayList<Interimaire> sl = new ArrayList<>();
		String nom, prenom, adresse, ville, contratAgence = null, codepostal="", secu = null;
		double salaire=0;
		Integer dureeMois = 0;
		boolean erreurcp = true;
		boolean erreurns =true;
		...
		for(int i=0;i<5; i++) {
			...
			System.out.println("Saisir la duree du contrat en mois de l'interimaire");
			while(Menus.sc.hasNext()&& dureeMois == 0)
			{
			...
			System.out.println("Saisir le contrat de l'Agence de l'interimaire");
			contratAgence = Menus.sc.next();
			Interimaire s = new Interimaire(nom, prenom, adresse, ville, codepostal, secu, salaire, dureeMois, contratAgence);
